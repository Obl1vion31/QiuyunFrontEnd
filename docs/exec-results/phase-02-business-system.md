...
1. 当前 `.env` 仍是示例数据库连接串，所以 Neon 尚未连接，迁移和真实保存均无法完成。
2. 查询数据库失败与数据库成功但没有记录，目前都会形成空表格；虽然前者有错误提示，但后续可考虑把错误状态与筛选区域进一步隔离，避免误解。
3. 页面当前一次读取全部排期，再在 Node 内存中筛选和汇总。MVP 数据量小时可用，数据增长后应把筛选、分页和统计下推到 PostgreSQL。
4. 当前只有新增和查看，没有编辑、删除或重复内容检查。错误录入只能直接在开发数据库清理。
5. `current_stage` 和 `completion_status` 在数据库中是文本字段，允许扩展，但数据库自身没有 CHECK 约束；当前完整性主要依赖 API 的 Zod 校验。
6. `updated_at` 当前只在创建时写入。以后实现编辑时，需要在更新语句中同步更新时间。
7. HTTP Basic Auth 无法区分操作者，不适合正式多人协作或敏感数据。
8. 尚未建立自动化测试；目前依靠类型检查、构建、迁移检查和手动 HTTP 验证。
