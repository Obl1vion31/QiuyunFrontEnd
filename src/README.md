...
排期相关的服务端代码位于 `src/db/`：`schema.ts` 分别定义帖子、内容版本和排期，`validation.ts` 定义新做帖、编辑、改帖与延期规则，`client.ts` 建立惰性数据库连接。`src/middleware.ts` 保护动态排期页和接口。一次性 Excel 导入脚本位于 `scripts/`，默认只预检，必须显式传入 `--apply` 才写数据库。数据库和认证变量不得进入客户端代码。
