...
配置里只有六类封面。这里使用 `%` 取余数，让它们循环两次，形成十二张视觉封面，而不用在数据文件中重复维护。
...
1. 先看 `data/home.ts`，认识页面实际使用的文字和列表。
2. 再看 `pages/index.astro` 的 HTML 部分，理解这些数据被放到了哪里。
3. 暂时跳过 CSS，阅读 `ContentTunnel.tsx` 的 Props、State 和 JSX。
4. 理解滚动进度后，再看 `ContentTunnel.module.css` 的层级与响应式。
5. 最后回到 `index.astro` 的全局 CSS，理解后半段布局。
